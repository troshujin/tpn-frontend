      ...
      <div class="flex flex-col xl:flex-row gap-6">
        ...
        <aside v-if="showFilters"
          ...
          <div>
            ...
            <div v-for="netId in uniqueNetworks" :key="netId" class="flex items-center gap-2">
              <input type="checkbox" :id="`network-${netId}`" v-model="filters.networks"
                :value="netId" class="accent-blue-600" />
              <label :for="`network-${netId}`" class="text-sm text-gray-800">{{ getNetworkName(netId)
                }}</label>
            </div>
          </div>
          ...
          <div>
            ...
            <div v-for="type in formats" :key="type" class="flex items-center gap-2">
              <input type="checkbox" :id="`type-${type}`" v-model="filters.formats"
                :value="type" class="accent-blue-600" />
              <label :for="`type-${type}`" class="text-sm text-gray-800">{{ type
                }}</label>
            </div>
          </div>
        ...
        <div class="flex-1">
          ...
          <div v-if="isImageGrid" class="flex flex-wrap gap-4">
            ...
            <p v-if="!filteredFiles.length"
              class="col-span-full text-center text-gray-500">No images found.</p>
          </div>
                  ...
                  <tr :title="file.name">
                    <td
                      ...
                      <div v-else-if="isVideo(file)" class="relative w-8 h-8">
                        <img :src="videoThumbnail(file.url)"
                        ...
                        <button @click="togglePlay(file)"
                          ...
                          <svg v-if="playingFile?.id === file.id"
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{
                      getAccessLevel(file.accessLevel).label }}</td>
                    <td
                      class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div class="flex items-center justify-end gap-3 h-full">
                        <button @click="$emit('editFile', file)"
                          class="text-blue-600 hover:text-blue-900 mr-3">
                          Manage
                        </button>
                        <button @click="$emit('removeFile', file)"
                  ...
                  <tr v-if="playingFile?.id === file.id">
                    <td colspan="7" class="px-6 pb-4">
                      ...
                      <div v-else-if="isVideo(file)">
                        <video v-if="playingFile?.id === file.id" controls autoplay
...
const clearFilters = () => {
  filters.value = { visibility: [], formats: [], minSize: 0, maxSize: 0, accessLevels: [], networks: [] };
  filterSlug.value = "";
};
...
function getNetworkName(networkId: string) {
  ...
  if (!pendingNetworkRequests.has(networkId)) {
    ...
    api.get<Network | undefined>(`/networks/${networkId}/details`)
      .then(response => {
...
const filteredFiles = computed(() => {
  ...
  return filesState.files.value.filter(file => {
    ...
    const matchesNetwork = filters.value.networks.length === 0 || filters.value.networks.includes(file.networkId);

    return matchesSearch && matchesMediaType && matchesVisibility && matchesMin && matchesMax && matchesAccessLevel && matchesNetwork;
  });
});
