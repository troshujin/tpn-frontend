...
<template>
  <div class="max-w-4xl mx-auto p-6 bg-white shadow-md rounded-lg overflow-hidden">
    <LoadingErrorComponent :loading="loading" :error="error ?? undefined" button-value="Go back" @button-action="router.go(-1)" :has-value="!!userProxy" />
    ...
    <div v-if="!loading && !error && userProxy" class="space-y-10">
      <div class="flex justify-between items-stretch gap-10">
        ...
        <section class="flex flex-col">
          ...
          <!-- Upload Modal -->
          <AddFileModal v-if="showUploadModal" :networkIds="networks.map(n => n.id)" @close="showUploadModal = false"
