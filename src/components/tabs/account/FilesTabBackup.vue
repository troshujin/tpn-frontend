<template>
  <div class="overflow-hidden rounded-lg bg-white p-6 shadow-md">
    <div class="flex flex-col gap-6">
      <!-- Toolbar -->
      <div class="flex w-full flex-col items-start justify-between gap-4">
        <div class="flex w-full flex-row items-start justify-between gap-4">
          <div class="flex justify-end gap-6">
            <button
              @click="showFilters = !showFilters"
              class="w-64 rounded-md bg-gray-200 py-2 text-sm text-gray-800 hover:bg-gray-300"
            >
              {{ showFilters ? 'Hide Filters' : 'Show Filters' }}
            </button>
            <h2 class="text-xl font-medium text-gray-800">Network Files</h2>
          </div>

          <div class="flex w-full flex-col gap-4 sm:w-auto sm:flex-row sm:items-center">
            <div class="relative w-full sm:w-64">
              <input
                type="text"
                v-model="filterSlug"
                placeholder="Filter files..."
                class="w-full rounded-md border border-gray-300 py-2 pl-3 pr-10 text-sm shadow-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
              <div class="pointer-events-none absolute inset-y-0 right-2 flex items-center">
                <svg
                  width="16"
                  height="16"
                  viewBox="0 0 24 24"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                  class="text-gray-500"
                >
                  <circle
                    cx="10"
                    cy="10"
                    r="6"
                    stroke="currentColor"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                  <path
                    d="M14.5 14.5L19 19"
                    stroke="currentColor"
                    stroke-width="2"
                    stroke-linecap="round"
                    stroke-linejoin="round"
                  />
                </svg>
              </div>
            </div>

            <button
              @click="$emit('addFile')"
              class="rounded-md bg-blue-600 px-4 py-2 text-white hover:bg-blue-700"
            >
              Add File
            </button>
          </div>
        </div>
      </div>

      <div class="flex flex-col gap-6 xl:flex-row">
        <!-- Sidebar Filters -->
        <aside
          v-if="showFilters"
          class="flex w-full flex-row flex-wrap items-start gap-4 rounded-lg border bg-white p-4 shadow-sm xl:w-64 xl:min-w-64 xl:flex-col"
        >
          <div>
            <h3 class="font-semibold text-gray-700">Network</h3>
            <div
              v-for="netId in uniqueNetworks"
              :key="netId"
              class="flex items-center gap-2"
            >
              <input
                type="checkbox"
                :id="`network-${netId}`"
                v-model="filters.networks"
                :value="netId"
                class="accent-blue-600"
              />
              <label
                :for="`network-${netId}`"
                class="text-sm text-gray-800"
                >{{ getNetworkName(netId) }}</label
              >
            </div>
          </div>

          <div>
            <h3 class="font-semibold text-gray-700">Access Level</h3>
            <div
              v-for="level in ACCESS_LEVELS"
              :key="level.value"
              class="flex items-center gap-2"
            >
              <input
                type="checkbox"
                :id="`access-${level.value}`"
                v-model="filters.accessLevels"
                :value="level.value"
                class="accent-blue-600"
              />
              <label
                :for="`access-${level.value}`"
                class="text-sm text-gray-800"
              >
                {{ level.label }}
              </label>
            </div>
          </div>

          <div>
            <h3 class="font-semibold text-gray-700">Format</h3>
            <div
              v-for="type in formats"
              :key="type"
              class="flex items-center gap-2"
            >
              <input
                type="checkbox"
                :id="`type-${type}`"
                v-model="filters.formats"
                :value="type"
                class="accent-blue-600"
              />
              <label
                :for="`type-${type}`"
                class="text-sm text-gray-800"
                >{{ type }}</label
              >
            </div>
          </div>

          <div>
            <h3 class="font-semibold text-gray-700">File Size (MB)</h3>
            <div class="flex items-center gap-2">
              <input
                type="number"
                v-model.number="filters.minSize"
                min="0"
                class="w-full rounded border px-2 py-1 text-sm"
                placeholder="Min"
              />
              <span>-</span>
              <input
                type="number"
                v-model.number="filters.maxSize"
                min="0"
                class="w-full rounded border px-2 py-1 text-sm"
                placeholder="Max"
              />
            </div>
          </div>

          <button
            @click="clearFilters"
            class="w-auto rounded border border-gray-300 bg-gray-100 px-3 py-2 text-sm text-gray-800 hover:bg-gray-200 xl:w-full"
          >
            Clear Filters
          </button>
        </aside>

        <!-- Main Content -->
        <div class="flex-1">
          <!-- Image Grid (only when filtering for images) -->
          <div
            v-if="isImageGrid"
            class="flex flex-wrap gap-4"
          >
            <div
              v-for="file in filteredFiles"
              :key="file.id"
              @click="$emit('editFile', file)"
              class="group relative cursor-pointer overflow-hidden rounded-lg border shadow-sm"
            >
              <img
                :src="file.url"
                class="h-40 w-full object-cover"
              />
              <div
                class="absolute inset-0 flex flex-col justify-end bg-black/50 p-2 text-xs text-white opacity-0 transition group-hover:opacity-100"
              >
                <div class="truncate">{{ file.name }}</div>
                <div>{{ readableSize(file.sizeBytes) }}</div>
              </div>
            </div>
            <p
              v-if="!filteredFiles.length"
              class="col-span-full text-center text-gray-500"
            >
              No images found.
            </p>
          </div>

          <!-- File Table -->
          <div
            v-else
            class="overflow-x-auto"
          >
            <table class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gray-50">
                <tr>
                  <th
                    class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500"
                  >
                    Name
                  </th>
                  <th
                    class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500"
                  >
                    Type
                  </th>
                  <th
                    class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500"
                  >
                    Extension
                  </th>
                  <th
                    class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500"
                  >
                    Size
                  </th>
                  <th
                    class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500"
                  >
                    Network
                  </th>
                  <th
                    class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500"
                  >
                    Visibility
                  </th>
                  <th
                    class="px-6 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500"
                  >
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-200 bg-white">
                <template
                  v-for="file in filteredFiles"
                  :key="file.id"
                >
                  <!-- Main file row -->
                  <tr :title="file.name">
                    <td
                      class="flex max-w-[200px] items-center gap-2 truncate whitespace-nowrap px-6 py-4 text-sm text-gray-900"
                    >
                      <img
                        v-if="file.mediaType === 'image'"
                        :src="file.url"
                        class="h-8 w-8 rounded object-cover"
                      />

                      <div
                        v-else-if="isVideo(file)"
                        class="relative h-8 w-8"
                      >
                        <img
                          :src="videoThumbnail(file.url)"
                          class="h-full w-full rounded object-cover"
                        />
                        <button
                          @click="togglePlay(file)"
                          class="absolute inset-0 flex items-center justify-center rounded bg-black/40 text-white"
                        >
                          <svg
                            v-if="playingFile?.id === file.id"
                            xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 24 24"
                            width="18"
                            height="18"
                            class="text-blue-600 shadow-md"
                          >
                            <rect
                              x="6"
                              y="6"
                              width="12"
                              height="12"
                              rx="1"
                              fill="currentColor"
                            />
                          </svg>
                          <svg
                            v-else
                            xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 24 24"
                            width="18"
                            height="18"
                            class="text-blue-600 shadow-md"
                          >
                            <path
                              d="M5 3v18l15-9L5 3z"
                              fill="currentColor"
                            />
                          </svg>
                        </button>
                      </div>

                      <button
                        v-else-if="isAudio(file)"
                        @click="togglePlay(file)"
                        :aria-pressed="playingFile?.id === file.id"
                        :title="playingFile?.id === file.id ? 'Stop' : 'Play'"
                        class="inline-flex h-8 w-8 items-center justify-center rounded-md hover:bg-blue-50 focus:outline-none focus:ring-2 focus:ring-blue-500"
                      >
                        <span class="sr-only">{{
                          playingFile?.id === file.id ? 'Stop audio' : 'Play audio'
                        }}</span>
                        <svg
                          v-if="playingFile?.id === file.id"
                          xmlns="http://www.w3.org/2000/svg"
                          viewBox="0 0 24 24"
                          width="18"
                          height="18"
                          class="text-blue-600"
                        >
                          <rect
                            x="6"
                            y="6"
                            width="12"
                            height="12"
                            rx="1"
                            fill="currentColor"
                          />
                        </svg>
                        <svg
                          v-else
                          xmlns="http://www.w3.org/2000/svg"
                          viewBox="0 0 24 24"
                          width="18"
                          height="18"
                          class="text-blue-600"
                        >
                          <path
                            d="M5 3v18l15-9L5 3z"
                            fill="currentColor"
                          />
                        </svg>
                      </button>
                      <span>{{ file.name }}</span>
                    </td>
                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                      {{ file.mediaType }}
                    </td>
                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                      {{ file.format }}
                    </td>
                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                      {{ readableSize(file.sizeBytes) }}
                    </td>
                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                      {{ getNetworkName(file.networkId) }}
                    </td>
                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                      {{ getAccessLevel(file.accessLevel).label }}
                    </td>
                    <td class="whitespace-nowrap px-6 py-4 text-right text-sm font-medium">
                      <div class="flex h-full items-center justify-end gap-3">
                        <button
                          @click="$emit('editFile', file)"
                          class="mr-3 text-blue-600 hover:text-blue-900"
                        >
                          Manage
                        </button>
                        <button
                          @click="$emit('removeFile', file)"
                          class="text-red-600 hover:text-red-900"
                        >
                          Remove
                        </button>
                      </div>
                    </td>
                  </tr>

                  <!-- Expansion row -->
                  <tr v-if="playingFile?.id === file.id">
                    <td
                      colspan="7"
                      class="px-6 pb-4"
                    >
                      <div v-if="isAudio(file)">
                        <audio
                          controls
                          autoplay
                          class="mt-2 w-full max-w-md"
                        >
                          <source
                            :src="file.url"
                            :type="`audio/${file.format}`"
                          />
                        </audio>
                      </div>
                      <div v-else-if="isVideo(file)">
                        <video
                          v-if="playingFile?.id === file.id"
                          controls
                          autoplay
                          class="mt-2 w-64 rounded shadow"
                        >
                          <source
                            :src="file.url"
                            :type="`video/${file.format}`"
                          />
                        </video>
                      </div>
                    </td>
                  </tr>
                </template>

                <tr v-if="!filteredFiles.length">
                  <td
                    colspan="7"
                    class="px-6 py-10 text-center text-gray-500"
                  >
                    No files found.
                  </td>
                </tr>
              </tbody>
            </table>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import useFiles from '@/composables/network/useFiles';
import { readableSize } from '@/lib/utils';
import type { Network, NetworkFile } from '@/types';
import { computed, onMounted, ref } from 'vue';
import { ACCESS_LEVELS, getAccessLevel } from '@/lib/accessLevels';
import { useAuthStore } from '@/stores/auth';
import api from '@/api/api';

const showFilters = ref(false);
const filterSlug = ref('');

const filesState = useFiles().fetchUserFiles;
const authStore = useAuthStore();

onMounted(async () => {
  const userProxy = await authStore.getUserProxy();
  if (!userProxy) return;
  filesState.execute(userProxy.user.id, userProxy.id);
});

const filters = ref({
  visibility: [] as string[],
  formats: [] as string[],
  minSize: 0,
  maxSize: 0,
  accessLevels: [] as number[],
  networks: [] as string[],
});

const clearFilters = () => {
  filters.value = {
    visibility: [],
    formats: [],
    minSize: 0,
    maxSize: 0,
    accessLevels: [],
    networks: [],
  };
  filterSlug.value = '';
};

// --- Media helpers ---
function isAudio(file: NetworkFile) {
  return file.mediaType === 'video' && ['mp3', 'wav', 'ogg'].includes(file.format);
}
function isVideo(file: NetworkFile) {
  return file.mediaType === 'video' && !isAudio(file);
}
function videoThumbnail(url: string) {
  // Cloudinary trick: swap to jpg preview
  return url.replace('/upload/', '/upload/w_200,h_120,c_fill/').replace(/\.[^.]+$/, '.jpg');
}

const playingFile = ref<NetworkFile | null>(null);
function togglePlay(file: NetworkFile) {
  if (playingFile.value?.id === file.id) {
    playingFile.value = null;
  } else {
    playingFile.value = file;
  }
}

const networkNames = ref<Record<string, string>>({});
const pendingNetworkRequests = new Set<string>();

function getNetworkName(networkId: string) {
  if (networkNames.value[networkId]) return networkNames.value[networkId];

  if (!pendingNetworkRequests.has(networkId)) {
    pendingNetworkRequests.add(networkId);
    api
      .get<Network | undefined>(`/networks/${networkId}/details`)
      .then((response) => {
        const network = response.data;
        networkNames.value[networkId] = network ? network.name : 'Unknown Network';
      })
      .catch(() => (networkNames.value[networkId] = 'Unknown Network'))
      .finally(() => pendingNetworkRequests.delete(networkId));
  }
  return 'Loading...';
}

// Extract formats
const formats = computed(() => {
  if (!filesState.data.value) return [];

  const types = new Set<string>();
  filesState.data.value.forEach((f) => {
    if (f.format) types.add(f.format.toLowerCase());
  });
  return Array.from(types).sort();
});

// Extract unique networks
const uniqueNetworks = computed(() => {
  if (!filesState.data.value) return [];

  const ids = new Set<string>();
  filesState.data.value.forEach((f) => ids.add(f.networkId));
  return Array.from(ids);
});

// Filtered files
const filteredFiles = computed(() => {
  if (!filesState.data.value) return [];

  const query = filterSlug.value.trim().toLowerCase();
  return filesState.data.value.filter((file) => {
    const name = file.name.toLowerCase();
    const format = file.format?.toLowerCase();
    const visibility = file.isPublic ? 'public' : 'private';
    const sizeMb = file.sizeBytes / 1024 / 1024;

    const matchesSearch = query === '' || name.includes(query) || format?.includes(query);
    const matchesMediaType =
      filters.value.formats.length === 0 || (format && filters.value.formats.includes(format));
    const matchesVisibility =
      filters.value.visibility.length === 0 || filters.value.visibility.includes(visibility);
    const matchesMin = !filters.value.minSize || sizeMb >= filters.value.minSize;
    const matchesMax = !filters.value.maxSize || sizeMb <= filters.value.maxSize;
    const matchesAccessLevel =
      filters.value.accessLevels.length === 0 ||
      filters.value.accessLevels.includes(file.accessLevel ?? 0);
    const matchesNetwork =
      filters.value.networks.length === 0 || filters.value.networks.includes(file.networkId);

    return (
      matchesSearch &&
      matchesMediaType &&
      matchesVisibility &&
      matchesMin &&
      matchesMax &&
      matchesAccessLevel &&
      matchesNetwork
    );
  });
});

// Switch to grid when only images are shown
const isImageGrid = computed(() => {
  if (!filteredFiles.value.length) return false;
  return filteredFiles.value.every((f) => f.mediaType === 'image');
});

defineEmits<{
  (e: 'addFile'): void;
  (e: 'editFile', file: NetworkFile): void;
  (e: 'removeFile', file: NetworkFile): void;
  (e: 'previewFile', file: NetworkFile): void;
}>();
</script>
