...
<template>
  <div class="p-6 bg-white shadow-md rounded-lg overflow-hidden">
    <LoadingErrorComponent :loading="loading" :error="error" button-value="Go back"
      @button-action="router.go(-1)" :has-value="!!network" />
    ...
    <div v-if="!loading && !error && network" class="space-y-10">
      ...
      <div class="flex justify-between items-stretch gap-10">
        <section class="flex-grow">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700">Callback
                URL</label>
            ...
            <div class="col-span-full">
              ...
              <p class="text-xs text-gray-500 mt-1">
                Toggle this on to make the network visible to anyone. Otherwise,
        ...
        <section class="flex flex-col mr-6">
          <h2 class="block text-sm font-medium text-gray-700 text-center">Network
            Image</h2>
          <div class="flex flex-col items-center flex-grow">
            ...
            <div v-else class="flex flex-col items-center justify-center flex-grow">
              <div
                class="h-24 w-24 rounded-full bg-gray-200 flex items-center justify-center text-gray-500">
                No Image
              </div>
              <button type="button"
          ...
          <AddFileModal v-if="showUploadModal" :network="network"
            @close="showUploadModal = false" @uploaded="handleImageUploaded"
            media-type="image" />
        </section>
      </div>

      <section v-if="!network.isSystemProtected"
        ...
        <p class="text-sm text-red-600 mb-4">
          Deleting this network is permanent and cannot be undone. Please proceed
          with caution.
        </p>
      ...
      <transition name="fade">
        <div v-if="hasChanges"
          class="fixed left-0 bottom-0 w-screen flex justify-center">
          <div
            ...
            <div class="flex gap-3">
              ...
              <button @click="saveChanges"
                class="px-4 py-2 text-sm rounded-md bg-green-600 hover:bg-green-700">
                Save Changes
              </button>
            </div>
          </div>
