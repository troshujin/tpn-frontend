        ...
        <form @submit.prevent="handleUpdate" class="space-y-4">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Parent
              Block</label>
            <SearchableSelect v-model="form.parentPageId" :options="parentOptions"
              label="Parent Block" :nullable="true" />
          </div>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Block
              Data</label>

            <div v-if="!editDataMode">
              <pre
                class="bg-gray-50 border rounded-md p-2 text-xs overflow-x-auto">{{ formattedJson }}</pre>
              <button type="button"
            ...
            <div v-else class="relative">
              <JsonEditorVue v-model="jsonEditorValue" :stringified="false"
    ...
    <div class="bg-white shadow rounded-lg p-4">
      ...
      <div class="flex gap-6 mt-6">
        ...
        <div v-if="tree.length">
          ...
          <div class="text-xs">
            <TreeNodeComponent v-for="root in tree" :key="root.id" :node="root"
              :current-id="pageBlockId" :network-id="networkId"
