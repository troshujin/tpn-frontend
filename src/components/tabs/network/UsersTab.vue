<template>
  <div class="bg-white shadow-md rounded-lg overflow-hidden p-6">
    <div class="flex justify-between items-center mb-6">
      ...
      <button v-if="!loading && !error" @click="$emit('addUser')"
    ...
    <div v-else class="overflow-x-auto">
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="networkUser in networkUsers" :key="networkUser.id">
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="flex items-center">
                <div class="h-10 w-10 flex-shrink-0">
                  <ProfileAvatar :userProxy="networkUser.userProxy" :size="10" />
                </div>
                <div class="ml-4">
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="flex flex-wrap gap-2">
                <span v-for="userRole in networkUser.networkUserRoles"
                  :key="userRole.role.id"
                  class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                  {{ userRole.role.name }}
                </span>
                <span v-if="!networkUser.networkUserRoles?.length"
                  class="text-sm text-gray-500">No roles assigned</span>
              </div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="flex flex-wrap gap-2" v-if="!networkUser.accessIncomplete">
                <span v-for="userAccess in networkUser.networkUserAccesses"
                ...
                <span v-if="!networkUser.networkUserAccesses?.length"
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
              <button @click="$emit('manageUser', networkUser)"
                class="text-blue-600 hover:text-blue-900 mr-3">
                Manage
              </button>
              <button @click="$emit('removeUser', networkUser)"
...
const { execute: fetchNetworkUsers, loading, error, data: networkUsers } = networkUsersState.fetchNetworkUsers;
