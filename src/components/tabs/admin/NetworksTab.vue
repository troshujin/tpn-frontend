  ...
  <div v-if="!!networks && !!mainNetwork && !loading" class="flex gap-4 flex-col">
    <div
      ...
      <div
        ...
        <h1 class="text-xs font-bold uppercase text-slate-500 tracking-widest">Global
        ...
        <div v-for="group in groupedEntitlements" :key="group.name"
          ...
          <div class="flex gap-2 flex-wrap">
            <div v-for="item in group.items" :key="item.label"
              ...
              <span class="text-slate-500 font-medium mr-2 text-nowrap">{{ item.label
              }}</span>
              <div>
                <span class="border-l border-slate-200 pl-2" :class="{
                  'text-slate-800': remainingLimits[item.key] > 10,
                ...
                <span class="font-bold text-slate-800 text-nowrap">
    ...
    <div v-if="mainNetwork"
      ...
      <div class="p-3 flex flex-wrap items-center justify-between gap-4">
        <div class="relative flex-1 min-w-[300px]">
          ...
          <input v-model="searchQuery" type="text" placeholder="Search name or ID..."
            ...
            <div v-for="key in allEntitlementKeys.filter(k => k.includes('Limit'))"
              ...
              <div class="flex justify-between items-center mb-2">
                ...
                <input type="checkbox" :checked="!!activeRangeFilters[key]"
                  @change="(event) => (event.target as HTMLInputElement).checked ? activeRangeFilters[key] = [0, mainNetwork!.entitlement![key]] : delete activeRangeFilters[key]"
                  class="rounded text-purple-600" />
              </div>

              <div v-if="activeRangeFilters[key]" class="space-y-3 pt-2">
                <div class="flex items-center gap-2">
                ...
                <input type="range" v-model.number="activeRangeFilters[key][0]"
                  :min="0" :max="mainNetwork!.entitlement![key]"
                  class="w-full h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-purple-600" />
                <input type="range" v-model.number="activeRangeFilters[key][1]"
                  :min="0" :max="mainNetwork!.entitlement![key]"
                  class="w-full h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-purple-600" />
                <div class="flex justify-between text-[10px] text-slate-400">
                  <span>Min: {{ activeRangeFilters[key][0] }}</span>
...
import type { EntitlementLimits, Network, NetworkEntitlement, SettableEntitlement } from '@/types';
...
const filteredNetworks = computed(() => {
  return allNetworks.value.filter(n => {
    ...
    // 3. Dynamic Boolean Filters (allowFiles, allowBlogs, etc.)
    ...
    // 4. Dynamic Range Filters (fileCountLimit, etc.)
    for (const [key, range] of Object.entries(activeRangeFilters.value)) {
      const val = (n.entitlement?.[key as keyof NetworkEntitlement] as number) || 0;
