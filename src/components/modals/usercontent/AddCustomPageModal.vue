  ...
  <modal-container title="Add Custom Page" @close="$emit('close')">
    <CreateUserContentContainer :is-submitting="isSubmitting"
      ...
      <div>
        ...
        <input id="pageName" v-model="name" type="text"
      ...
      <div>
        ...
        <input id="pageSlug" v-model="slug" type="text"
          placeholder="Enter page slug"
          class="block w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm text-gray-900 shadow-sm focus:border-blue-600 focus:ring focus:ring-blue-100 disabled:bg-gray-100 disabled:text-gray-500 transition-all"
          required />
        <p class="mt-1 text-xs text-gray-500">Information which should make it easier
          to look up.</p>
      </div>
...
function handleSubmit(form: CreateUserContentForm) {
  emit('submit', form.networkId, { name: name.value.trim(), slug: slug.value.trim(), accessLevel: form.accessLevel });
}
</script>
