<template>
  <Sidebar
    :nav-items="navItems"
    :base-path="`/networks/${networkId}/manage`"
    sub-label-key="name"
  >
    <template #header>
      <RouterLink
        :to="`/networks/${networkId}/`"
        class="flex items-center gap-2 text-gray-700 transition hover:text-blue-700"
      >
        <!-- Back arrow icon -->
        <svg
          class="h-4 w-4"
    ...
    items: [
      ...
      },
      {
        ...
        subItems: historyStore.data.configurations
          .map((cp) => ({ page: cp.id, label: cp.key }))
          .slice()
          .reverse(),
